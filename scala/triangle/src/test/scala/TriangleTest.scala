...
class TriangleTest extends FunSuite with Matchers {
  ...
  test("degenerate - true if sum of first two is equal to third") {
    Triangle(1, 1, 2).degenerate should be (true)
  }

  test("degenerate - true if sum of last two is equal to first") {
    Triangle(2, 1, 1).degenerate should be (true)
  }

  test("degenerate - true if sum of first and last is equal to second") {
    Triangle(1, 2, 1).degenerate should be (true)
  }

  test("degenerate - false if all sides are 0") {
    Triangle(0, 0, 0).degenerate should be (false)
  }
}
